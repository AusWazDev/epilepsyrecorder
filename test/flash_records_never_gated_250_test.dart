...
///
/// ⭐ AND THIS FILE HOLDS EXACTLY ONE TEST, for the same reason one level down:
/// with 5 and 5b together, 5b was the SECOND test in the process and returned
/// -1 in its turn. The rule is not "one file per concern" -- it is ONE PREFS
/// STATE PER PROCESS, and a file with two prefs-dependent tests already breaks
/// it. The sibling is `flash_records_never_gated_250_test.dart`.
///
...
void main() {
  ...
  testWidgets('5b. and at 250 ms, where onsets ARE suppressed',
      (tester) async {
    ...
    // ignore: avoid_print
    print('RECORDS 250ms taps=$taps onsets=$onsets before=$before after=$after');
    expect(after - before, taps,
        reason: 'suppressing a FLASH must never suppress a RECORD');
    expect(onsets, lessThan(taps),
        reason: 'and onsets really were suppressed here, so the check above '

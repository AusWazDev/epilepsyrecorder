...
| # | Item |
|---|------|
| C1 | ⚠️ **SUPERSEDED 8 Sep 2026 — the claim below is left as written and was true of 26 Apr 2026. The 30 August and 7 September 2026 capture passes BOTH forced portrait successfully** (`captures/INDEX.md`: *"Orientation: Portrait, forced. `accelerometer_rotation 0` + `user_rotation 0`"*), and the working rule is now recorded in this file's 7 September session: **`wm size` inherits the rotation in force, so the display must already be portrait when the override lands.** — **MER will not render in portrait on the Teclast P30.** No `screenOrientation` in `AndroidManifest.xml`, no `setPreferredOrientations` anywhere in `lib/`, yet the app stays at 1280x800 through `user_rotation 0`, a `wm size 800x1280` override and a relaunch, while the launcher rotates. **Cause unknown and deliberately not guessed.** Consequence: every device verification this session tested ONE orientation, and the landscape-only defects already found (`1c6c359`, the Save button under the nav bar) are exactly the class this hides. |

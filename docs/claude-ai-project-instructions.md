...
- Repo: C:\dev\WordFind-Adventure (name predates the rebrand — do not rename).
  Branch `main`. Listed in the C:\dev\CLAUDE.md project table (added 20 Aug 2026).
...
- Platforms: see STORE PRESENCE below. Apple and Microsoft released; Google Play
  still in release testing and not publicly listed.
...
- Released on all three stores — Apple, Google Play and Microsoft. See STORE
  PRESENCE below. Last App Store release was 1.0.2; the repo is ahead at 1.1.0+5.
...
- App Store Connect — BOTH apps
- Google Play Console — BOTH apps
- Microsoft Partner Center — BOTH apps
- AdMob — SoundFind
- Google Groups soundfind-testers@googlegroups.com — SoundFind closed testing
- ClickUp — task tracking
- Sentry — BOTH apps
- AppFollow — recommended, not confirmed as set up

## STORE PRESENCE (developer-supplied, corrected 20 Aug 2026)

Console state cannot be read from a repo. Treat the table as the developer's
statement and confirm in the consoles before relying on it for anything
submitted.

| Store | MER | SoundFind |
|---|---|---|
| Apple App Store | Released, v1.0.2 (ID 6764339880) | Released, v1.1.0 (ID 6769255354) |
| Google Play | Released, v1.0.2 | **Closed testing Alpha — not public** |
| Microsoft Store | Released, v1.0.1 (ID 9PMJ09CDSL6K) | Released, v1.0.1 (ID 9PG86ZDTB3P0) |
| TestFlight | v1.0.3+4 external testing | — |

Versions above are from memory files dated 7–10 Jun 2026 and are the most
specific record available; they are NOT audited. Note MER's Microsoft Store sits
a version behind Apple and Google Play, and the repo is further ahead again at
1.1.0+5.

SoundFind Google Play needs 12 opted-in testers for 14 consecutive days; 3 were
recorded as added, so roughly 9 short. Opt-in link:
https://play.google.com/apps/testing/au.com.uniquegames.soundfind

Both apps are on all three stores. The single exception is SoundFind on Google
Play, which is still in testing and needs 12 opted-in testers for 14 consecutive
days before production unlocks.

An earlier version of these instructions listed Google Play as SoundFind-only and
omitted the Microsoft Store entirely. Both were wrong.
```
